    ...
    def __init__(self, root_dir, patient_list, is_train=True):
        ...
        for patient in patient_list:
            ...
            for sid in common_ids:
                ...
                if mask_arr.sum() == 0:
                    if random.random() > 0.3:  # keep ~30% no-liver slices
...
def dice_loss(pred_prob, target, eps=1e-6):
    ...
    inter = (pred_prob * target).sum(dim=1)
...
def combined_loss(pred, target, epoch):
    ...
    bce  = bce_loss(pred, target) # <-- this is FOCAL BCE

    # Strong Dice supervision
    loss = 0.8 * dice + 0.2 * bce
     
     # ---- Boundary curriculum learning ----
    if epoch >= 20:
        loss += 0.005 * boundary_loss(pred, target)

    # No-liver penalty (keep it)
    liver_present = (target.sum(dim=(1,2,3)) > 0).float()
    no_liver_penalty = ((pred_prob > 0.4).float().sum(dim=(1,2,3)) > 0).float()
    loss += 0.02 * (no_liver_penalty * (1 - liver_present)).mean()
...
def validate(model, loader):
    ...
    with torch.no_grad():
        for imgs, masks in loader:
            ...
            preds = model(imgs)
            loss = combined_loss(preds, masks, epoch=999)
