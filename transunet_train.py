...
EPOCHS = 200             # you can increase later if you have time
LR = 3e-5
...
class LiverPatientDataset(Dataset):
    def __init__(self, root_dir, patient_list, is_train=True):
        self.samples = []
        self.is_train = is_train  # Only augment if training
        ...
        for patient in patient_list:
            ...
            img_files = sorted(os.listdir(ct_dir))
            ...
            for name in common:
                ...
                # OPTIMIZATION: Peek at mask to see if it contains liver
                # We open, verify, and close immediately.
                with Image.open(mask_path).convert("L") as m:
                    mask_arr = np.array(m)
                    # Keep if there is at least 1 white pixel
                    
                    img_path = os.path.join(ct_dir, name)
...
def combined_loss(pred, target):
     """
    Nash-inspired dynamic loss weighting:
    Dice loss and BCE loss act as competing players.
    Their weights adapt dynamically to avoid dominance.
    """
    ...
    # -------- Nash-inspired dynamic weights --------
    # If Dice is poor → give Dice more weight
     lambda_dice = torch.clamp(1.0 - dice.detach(), 0.3, 0.7)
     lambda_bce  = 1.0 - lambda_dice

    # Weighted loss
     loss = lambda_dice * dice + lambda_bce * bce

    # -------- No-liver penalty (keep your logic) ----
     liver_present = (target.sum(dim=(1,2,3)) > 0).float()
     no_liver_penalty = ((pred_prob > 0.3).float().sum(dim=(1,2,3)) > 0).float()
     no_liver_penalty = no_liver_penalty * (1 - liver_present)

     return loss + 0.5 * no_liver_penalty.mean()
